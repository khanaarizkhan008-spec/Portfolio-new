          ...
          {skills.length === 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
              {["Next.js", "React", "TypeScript", "Python", "TailwindCSS", "Prisma", "PostgreSQL", "Figma", "Docker", "Go", "Git", "OpenAI"].map((name) => (
                <div
                  key={name}
                  className="p-4 sm:p-6 rounded-3xl bg-zinc-900/60 border border-white/10 hover:border-white/30 hover:bg-zinc-900 transition-all duration-300 flex items-center gap-3 sm:gap-4 group"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              {categories.map((cat) => {
                ...
                return (
                  <div
                    key={cat}
                    className="p-6 sm:p-8 md:p-9 rounded-3xl bg-zinc-900/60 border border-white/10 space-y-5 sm:space-y-6 hover:border-white/20 transition-colors shadow-2xl"
                  >
                    <div className="flex items-center gap-3 sm:gap-3.5">
                      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-white/5 border border-white/10 text-zinc-200 flex items-center justify-center">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
                      {catSkills.map((skill) => (
                        <div
                          key={skill.id}
                          className="p-3.5 sm:p-4 rounded-2xl bg-zinc-950/60 border border-white/10 hover:border-white/30 transition-all duration-200 flex items-center justify-between group"
                        >
                          <span className="text-xs sm:text-sm font-bold text-zinc-200 group-hover:text-white transition-colors truncate pr-2">
                            {skill.name}
                          </span>
                          <span className="text-[9px] sm:text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 rounded-full bg-white/5 text-zinc-400 font-semibold border border-white/10 shrink-0">
                            Active
                          </span>
                        </div>
                      ))}
